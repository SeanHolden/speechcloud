module IvonaSpeechCloud  
  module Pronunciation
    ...
    # lang_id example: 'gb'
    def self.list_pronunciation_rules( lang_id )
    ...
    def self.add_pronunciation_rule( lang_id, stat, key, value )
      token  = Auth.get_token
      md5    = GetMd5.formula(token)
      params = {  token:token,
                  md5:md5,
    ...
    def self.modify_pronunciation_rule( id, lang_id, stat, key, value )
      token  = Auth.get_token
      md5    = GetMd5.formula(token)
      params = [  "token=#{token}",
                  "md5=#{md5}",
    ...
    def self.delete_pronunciation_rule( id, lang_id )
      token  = Auth.get_token
      md5    = GetMd5.formula(token)
      params = [  "token=#{token}",
                  "md5=#{md5}",
